...
/* io channel widths */
 ...
 if (ny > 1) {
  ...
  /* No zero width channels */
    chan_width_x[1] = my_max(chan_width_x[1],1);

    for (i=1;i<ny-1;i++) {
       x = (float) i/((float) (ny-2.));
       chan_width_x[i+1] = (int) floor (cfactor*comp_width(&chan_x_dist, x,
                   separation) + 0.5);
       chan_width_x[i+1] = my_max(chan_width_x[i+1],1);
    }
 }
 ...
 if (nx > 1) {
    ...
    chan_width_y[1] = my_max(chan_width_y[1],1);

    for (i=1;i<nx-1;i++) {
       x = (float) i/((float) (nx-2.));
       chan_width_y[i+1] = (int) floor (cfactor*comp_width(&chan_y_dist, x,
                 separation) + 0.5);
       chan_width_y[i+1] = my_max(chan_width_y[i+1],1);
    }
 }
