 ...
 * optional.                                                       */
  ...
  if (argc < 5) {
    my_printf("Usage:  vpr circuit.net fpga.arch placed.out routed.out "
	   "[Options ...]\n\n");
    my_printf("General Options:  [-nodisp] [-auto <int>] [-route_only]\n");
    my_printf("\t[-place_only] [-timing_analyze_only_with_net_delay <float>]\n");
    my_printf("\t[-aspect_ratio <float>] [-nx <int>] [-ny <int>] [-fast]\n");
    my_printf("\t[-full_stats] [-timing_analysis on | off]\n");

    my_printf("\nPlacer Options: \n");
    my_printf("\t[-place_algorithm bounding_box | net_timing_driven | path_timing_driven]\n");
    my_printf("\t[-init_t <float>] [-exit_t <float>]\n");
    my_printf("\t[-alpha_t <float>] [-inner_num <float>] [-seed <int>]\n");
    my_printf("\t[-place_cost_exp <float>] [-place_cost_type linear | "
	   "nonlinear]\n");
    my_printf("\t[-place_chan_width <int>] [-num_regions <int>] \n");
    my_printf("\t[-fix_pins random | <file.pads>]\n");
    my_printf("\t[-enable_timing_computations on | off]\n");
    my_printf("\t[-block_dist <int>]\n");

    my_printf("\nPlacement Options Valid Only for Timing-Driven Placement:\n");
    my_printf("\t[-timing_tradeoff <float>]\n");
    my_printf("\t[-recompute_crit_iter <int>]\n");
    my_printf("\t[-inner_loop_recompute_divider <int>]\n");
    my_printf("\t[-td_place_exp_first <float>]\n");
    my_printf("\t[-td_place_exp_last <float>]\n");

    my_printf("\nRouter Options:  [-max_router_iterations <int>] "
	   "[-bb_factor <int>]\n");
    my_printf("\t[-initial_pres_fac <float>] [-pres_fac_mult <float>]\n"
...
/* Now get any optional arguments.      */
  ...
  while (i < argc) {
    ...
    if (strcmp(argv[i],"-aspect_ratio") == 0) {
      ...
      *aspect_ratio = read_float_option (argc, argv, i);
      ...
      if (*aspect_ratio <= 0.) {
	my_printf("Error:  Aspect ratio must be > 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -auto value must be between 0 and 2.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error: -recompute_crit_iter must be 1 or more \n");
	exit (1);
      }
      i += 2;
      continue;
	...
	my_printf("Error: -inner_loop_recompute_divider must be 1 or more \n");
	exit (1);
      }
      i += 2;
      continue;
    ...
    if (strcmp(argv[i],"-fix_pins") == 0) {
      ...
      if (argc <= i+1) {
	my_printf("Error:  -fix_pins option requires a string parameter.\n");
	exit (1);
      }
    ...
    if (strcmp(argv[i],"-nx") == 0) {
      ...
      *user_sized = TRUE;
      ...
      if (nx <= 0) {
	my_printf("Error:  -nx value must be greater than 0.\n");
	throw std::runtime_error("1");
      }
    ...
    if (strcmp(argv[i],"-ny") == 0) {
      ...
      *user_sized = TRUE;
      ...
      if (ny <= 0) {
	my_printf("Error:  -ny value must be greater than 0.\n");
	throw std::runtime_error("1");
      }
    ...
    if (strcmp (argv[i],"-timing_analysis") == 0) {
      if (argc <= i+1) {
	my_printf ("Error:  -timing_analysis option requires a string "
		"parameter.\n");
	exit (1);
      }
	...
	*timing_analysis_enabled = FALSE;
	...
	my_printf("Error:  -timing_analysis must be on or off.\n");
	exit (1);
      }
	...
	my_printf("Error:  -timing_analyze_only_with_net_delay value must "
	       "be > 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -init_t value must be nonnegative 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -alpha_t value must be between 0. and 1.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -exit_t value must be greater than 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -inner_num value must be greater than 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -place_cost_exp value must be nonnegative.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -td_place_exp_first value must be nonnegative.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -td_place_exp_last value must be nonnegative.\n");
	throw std::runtime_error("1");
      }
    ...
    if (strcmp(argv[i],"-place_algorithm") == 0) {
      ...
      if (argc <= i+1) {
	my_printf("Error:  -place_algorithm option requires a "
               "string parameter.\n");
	exit (1);
      }
	...
	my_printf("Error:  -place_algorithm must be bounding_box, "
	...
	my_printf("Error:  -timing_tradeoff value must be nonnegative.\n");
	throw std::runtime_error("1");
      }
    ...
    if (strcmp(argv[i],"-enable_timing_computations") == 0) {
      if (argc <= i+1) {
	my_printf ("Error:  -enable_timing_computations option requires a string "
		"parameter.\n");
	exit (1);
      }
      if (strcmp(argv[i+1], "on") == 0) {
	...
	my_printf("Error:  -enable_timing_computations must be on or off.\n");
	exit (1);
      }
      i += 2;
      continue;
	...
	my_printf("Error:  -block_dist value must be nonnegative.\n");
	throw std::runtime_error("1");
      }
    ...
    if (strcmp(argv[i],"-place_cost_type") == 0) {
      ...
      if (argc <= i+1) {
	my_printf("Error:  -place_cost_type option requires a "
               "string parameter.\n");
	exit (1);
      }
	...
	my_printf("Error:  -place_cost_type must be linear or nonlinear.\n");
	exit (1);
      }
	...
	my_printf("Error:  -num_regions value must be greater than 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -place_chan_width value must be greater than 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -max_router_iterations value is less than 0.\n");
	throw std::runtime_error("1");
      }
	...
	my_printf("Error:  -bb_factor is less than 0.\n");
	exit (1);
      }
    ...
    if (strcmp(argv[i],"-router_algorithm") == 0) {
      ...
      if (argc <= i+1) {
	my_printf("Error:  -router_algorithm option requires a string "
	       "parameter.\n");
	exit (1);
      }
	...
	my_printf("Error:  -router_algorithm must be breadth_first or "
	       "timing_driven.\n");
	exit (1);
      }
	...
	my_printf("Error:  -first_iter_pres_fac must be nonnegative.\n");
	exit (1);
      }
	...
	my_printf("Error:  -initial_pres_fac must be greater than 0.\n");
	exit (1);
      }
	...
	my_printf("Error:  -pres_fac_mult must be greater than "
               "0.\n");
	exit (1);
      }
	...
	my_printf("Error:  -acc_fac must be nonnegative.\n");
	exit (1);
      }
	...
	my_printf("Error:  -astar_fac must be nonnegative.\n");
	exit (1);
      }
    ...
    if (strcmp(argv[i],"-max_criticality") == 0) {
	  ...
	  >= 1.) {
	my_printf("Error:  -max_criticality must be greater than or equal\n"
	...
	my_printf("Error:  -criticality_exp must be non-negative.\n");
	exit (1);
      }
    ...
    if (strcmp(argv[i],"-base_cost_type") == 0) {
      ...
      if (argc <= i+1) {
	my_printf("Error:  -base_cost_type option requires a string "
	       "parameter.\n");
	exit (1);
      }
	...
	my_printf("Error:  -base_cost_type must be intrinsic_delay, "
	...
	my_printf("Error:  -bend_cost cannot be less than 0.\n");
	exit (1);
      }
    ...
    if (strcmp(argv[i],"-route_type") == 0) {
      ...
      if (argc <= i+1) {
	my_printf("Error:  -route_type option requires a "
               "string parameter.\n");
	exit (1);
      }
	...
	my_printf("Error:  -route_type must be global or detailed.\n");
	exit (1);
      }
	...
	my_printf("Error:  -route_chan_width value must be greater than 0.\n");
	throw std::runtime_error("1");
      }

      if (router_opts->fixed_channel_width >= MAX_CHANNEL_WIDTH) {
	my_printf ("Error:  -router_chan_width value must be less than %d."
		"\n", MAX_CHANNEL_WIDTH);
	exit (1);
      }
	...
	my_printf("Error:  Both -route_only and -place_only specified.\n");
	exit (1);
      }
    ...
    my_printf("Error:  Unrecognized flag: %s.  Aborting.\n",argv[i]);
    throw std::runtime_error("1");
  ...
  if (placer_opts->place_cost_type == NONLINEAR_CONG && *operation !=
    ...
    my_printf("Error:  Replacing using the nonlinear congestion option\n");
    my_printf("        for each channel width makes sense only for full "
	   "place and route.\n");
    exit (1);
  }
  ...
  if (placer_opts->place_cost_type == NONLINEAR_CONG &&
    ...
    my_printf("Error: Cannot use nonlinear placement with \n"
  ...
  if (router_opts->route_type == GLOBAL &&
     ...
     */
    my_printf("Warning: Using global routing with timing-driven placement.\n");
    my_printf("\tThis is allowed, but strange, and circuit speed will suffer.\n");
  }
  ...
  if ( *timing_analysis_enabled == FALSE  &&
     (placer_opts->place_algorithm == NET_TIMING_DRIVEN_PLACE ||
      ...
      /*may work, not tested*/
      my_printf("Error: timing analysis must be enabled for timing-driven placement\n");
      exit (1);
  }


  if (*operation == ROUTE_ONLY && placer_opts->pad_loc_type == USER) {
    my_printf ("Error:  You cannot specify both a full placement file and \n");
    my_printf ("        a pad location file.\n");
    exit (1);
  }

  if (router_opts->fixed_channel_width != NO_FIXED_CHANNEL_WIDTH &&
      *verify_binary_search) {
    my_printf ("Error:  Routing on a fixed channel width (%d tracks) "
	    "specified.\n", router_opts->fixed_channel_width);
    my_printf ("        There is no binary search to verify, so you \n"
  ...
  if (*operation == ROUTE_ONLY || *operation == PLACE_AND_ROUTE) {
	...
	*timing_analysis_enabled == FALSE) {
      my_printf ("Error:  Cannot perform timing-driven routing when timing "
	...
	!= DEMAND_ONLY) {
      my_printf ("Error:  base_cost_type must be demand_only when timing "
	      "analysis is disabled.\n");
      exit (1);
    }
  }
  ...
  if (*operation == TIMING_ANALYSIS_ONLY && *timing_analysis_enabled ==
      FALSE) {
    my_printf ("Error:  -timing_analyze_only_with_net_delay option requires\n"
  ...
  if (*aspect_ratio != 1.) {
    my_printf("\tFPGA will have a width/length ratio of %g.\n",
	   *aspect_ratio);
  }

  if (*user_sized == TRUE) {
    my_printf("\tThe FPGA size has been specified by the user.\n");
    ...
    if (ny == 0) {
      ...
      my_printf("not match.  Note that aspect ratio does not have to\n");
      my_printf("be specified if both nx and ny are specified.\n");
      exit (1);
    }
  }

  if (*full_stats == TRUE) {
    my_printf("\tA full statistical report will be printed.\n");
  }

  if (*operation == ROUTE_ONLY) {
    my_printf ("\tRouting will be performed on an existing placement.\n");
  } else if (*operation == PLACE_ONLY) {
    my_printf ("\tThe circuit will be placed but not routed.\n");
  } else if (*operation == PLACE_AND_ROUTE) {
    my_printf ("\tThe circuit will be placed and routed.\n");
  } else if (*operation == TIMING_ANALYSIS_ONLY) {
    my_printf ("\tOnly timing analysis (assuming a constant net delay) will "
	    "be performed.\n");
  }
  ...
  if (*operation == PLACE_ONLY || *operation == PLACE_AND_ROUTE) {
    ...
    if (placer_opts->place_algorithm == NET_TIMING_DRIVEN_PLACE)
      my_printf("\tPlacement algorithm is net_timing_driven\n");
    if (placer_opts->place_algorithm == PATH_TIMING_DRIVEN_PLACE)
      my_printf("\tPlacement algorithm is path_timing_driven\n");
    if (placer_opts->place_algorithm == NET_TIMING_DRIVEN_PLACE ||
	placer_opts->place_algorithm == PATH_TIMING_DRIVEN_PLACE) {

      my_printf("\tTradeoff between bounding box and critical path: %g\n",placer_opts->timing_tradeoff);
      my_printf("\tLower bound assumes block distance: %d\n",placer_opts->block_dist);

      my_printf("\tRecomputing criticalities every %d temperature changes\n",
	     placer_opts->recompute_crit_iter);
      my_printf("\tInner loop computes criticalities every move_lim/%d moves\n",
	     placer_opts->inner_loop_recompute_divider);

      my_printf("\tExponent starting value used in timing-driven cost function is %g\n",
	     placer_opts->td_place_exp_first);
      my_printf("\tExponent final value used in timing-driven cost function is %g\n",
	     placer_opts->td_place_exp_last);
    }

    else if (placer_opts->place_algorithm == BOUNDING_BOX_PLACE) {
      my_printf("\tPlacement algorithm is bounding_box\n");
      if (placer_opts->enable_timing_computations) {
         my_printf("\tPlacement algorithm will generate timing estimates\n"
	    "\t(estimates do not affect the placement in bounding_box mode)\n");
      }
    }

    if (annealing_sched->type == AUTO_SCHED) {
      my_printf("\tAutomatic annealing schedule selected.\n");
      my_printf("\tNumber of moves in the inner loop is (num_blocks)^4/3 "
	     "* %g\n", annealing_sched->inner_num);
    }
    else {
      my_printf("\tUser annealing schedule selected with:\n");
      my_printf("\tInitial Temperature: %g\n",annealing_sched->init_t);
      my_printf("\tExit (Final) Temperature: %g\n",annealing_sched->exit_t);
      my_printf("\tTemperature Reduction factor (alpha_t): %g\n",
    ...
    if (placer_opts->place_cost_type == NONLINEAR_CONG) {
      my_printf("\tPlacement cost type is nonlinear congestion.\n");
      my_printf("\tCongestion will be determined on a %d x %d array.\n",
	     placer_opts->num_regions, placer_opts->num_regions);
      if (do_one_nonlinear_place == TRUE) {
	placer_opts->place_freq = PLACE_ONCE;
	my_printf("\tPlacement will be performed once.\n");
	my_printf("\tPlacement channel width factor = %d.\n",
	       placer_opts->place_chan_width);
      }
      else {
	placer_opts->place_freq = PLACE_ALWAYS;
	my_printf("\tCircuit will be replaced for each channel width "
	       "attempted.\n");
      }
    }
      ...
      placer_opts->place_freq = PLACE_ONCE;
      my_printf("\tPlacement cost type is linear congestion.\n");
      my_printf("\tPlacement will be performed once.\n");
      my_printf("\tPlacement channel width factor = %d.\n",
	     placer_opts->place_chan_width);
      my_printf("\tExponent used in placement cost: %g\n",
	     placer_opts->place_cost_exp);
    }

    if (placer_opts->pad_loc_type == RANDOM) {
      my_printf("\tPlacer will fix the IO pins in a random configuration.\n");
    }
    else if (placer_opts->pad_loc_type == USER) {
        my_printf ("\tPlacer will fix the IO pins as specified by file %s.\n",
    ...
    my_printf("\tInitial random seed: %d\n", seed);
    my_srandom(seed);
  ...
  if (*operation == ROUTE_ONLY || *operation == PLACE_AND_ROUTE) {
    ...
    if (router_opts->route_type == GLOBAL)
      my_printf("\tOnly GLOBAL routing will be performed.\n");
    else
      my_printf("\tCombined GLOBAL + DETAILED routing will be performed.\n");

    if (router_opts->router_algorithm == BREADTH_FIRST)
      my_printf ("\tRouter algorithm:  breadth first.\n");
    else
      my_printf ("\tRouter algorithm:  timing driven.\n");

    my_printf("\tThe router will try at most %d iterations.\n",
	   router_opts->max_router_iterations);
    my_printf("\tRoutings can go at most %d channels outside their "
    ...
    my_printf("\tCost of a bend (bend_cost) is %g.\n", router_opts->bend_cost);

    my_printf("\tFirst iteration sharing penalty factor (first_iter_pres_fac):"
	   " %g\n", router_opts->first_iter_pres_fac);
    my_printf("\tInitial (2nd iter.) sharing penalty factor (initial_pres_fac):"
	   " %g\n", router_opts->initial_pres_fac);
    my_printf("\tSharing penalty growth factor (pres_fac_mult): %g\n",
	   router_opts->pres_fac_mult);
    my_printf("\tAccumulated sharing penalty factor (acc_fac): %g\n",
	   router_opts->acc_fac);
    ...
    my_printf ("\tBase_cost_type:  ");
    if (router_opts->base_cost_type == INTRINSIC_DELAY)
      my_printf ("intrinsic delay.\n");
    else if (router_opts->base_cost_type == DELAY_NORMALIZED)
      my_printf ("delay normalized.\n");
    else if (router_opts->base_cost_type == DEMAND_ONLY)
      my_printf ("demand only.\n");

    if (router_opts->router_algorithm == TIMING_DRIVEN) {
      my_printf ("\tSearch aggressiveness factor (astar_fac): %g\n",
	      router_opts->astar_fac);
      my_printf ("\tMaximum sink criticality (max_criticality): %g\n",
	      router_opts->max_criticality);
      my_printf ("\tExponent for criticality computation (criticality_exp): "
	      "%g\n", router_opts->criticality_exp);
    }

    if (router_opts->fixed_channel_width == NO_FIXED_CHANNEL_WIDTH) {
      my_printf ("\tRouter will find the minimum number of tracks "
	      "required to route.\n");
    }
    else {
      my_printf ("\tRouter will attempt routing only with a channel width"
	      " factor of %d.\n", router_opts->fixed_channel_width);
    }
    ...
    if (*verify_binary_search) {
      ...
      my_printf("\tRouter will verify that binary search yields min. "
	     "channel width.\n");
    }
  ...
  if (*operation == TIMING_ANALYSIS_ONLY) {
    my_printf ("\tNet delay value for timing analysis: %g (s)\n",
	    *constant_net_delay);
  }
 ...
 * both the algorithms to be used and the FPGA architecture.                */
  ...
  my_printf("Reading the FPGA architectural description from %s.\n",
	 arch_file);
  ...
  my_printf("Pins per clb: %d.  Pads per row/column: %d.\n", pins_per_clb, io_rat);
  my_printf("Subblocks per clb: %d.  Subblock LUT size: %d.\n",
	 subblock_data_ptr->max_subblocks_per_block,
	 subblock_data_ptr->subblock_lut_size);

  if (route_type == DETAILED) {
    if (det_routing_arch->Fc_type == ABSOLUTE)
      my_printf("Fc value is absolute number of tracks.\n");
    else
      my_printf("Fc value is fraction of tracks in a channel.\n");

    my_printf("Fc_output: %g.  Fc_input: %g.  Fc_pad: %g.\n",
           det_routing_arch->Fc_output, det_routing_arch->Fc_input,
           det_routing_arch->Fc_pad);

    if (det_routing_arch->switch_block_type == SUBSET)
      my_printf("Switch block type: Subset.\n");
    else if (det_routing_arch->switch_block_type == WILTON)
      my_printf("Switch_block_type: WILTON.\n");
    else
      my_printf ("Switch_block_type: UNIVERSAL.\n");

    my_printf ("Distinct types of segments: %d.\n",
            det_routing_arch->num_segment);
    my_printf ("Distinct types of user-specified switches: %d.\n",
            det_routing_arch->num_switch - 2);
  }
  my_printf ("\n");


  my_printf("Reading the circuit netlist from %s.\n",net_file);
  read_net (net_file, subblock_data_ptr);
  my_printf("Successfully read %s.\n", net_file);
  my_printf("%d blocks, %d nets, %d global nets.\n", num_blocks, num_nets,
	 num_globals);
  my_printf("%d clbs, %d inputs, %d outputs.\n", num_clbs, num_p_inputs,
	 num_p_outputs);
 ...
 *  know num_blocks.                                        */
  ...
  my_printf("The circuit will be mapped into a %d x %d array of clbs.\n\n",
	 nx, ny);

  if (place_cost_type == NONLINEAR_CONG && (num_regions > nx ||
					    num_regions > ny)) {
    my_printf("Error:  Cannot use more regions than clbs in placement cost "
	   "function.\n");
    throw std::runtime_error("1");
  }
...
static int read_int_option (int argc, char *argv[], int iarg) {
  ...
  if (num_read != 1) {
    my_printf("Error:  %s option requires an integer parameter.\n\n", argv[iarg]);
    throw std::runtime_error("1");
  }
...
static float read_float_option (int argc, char *argv[], int iarg) {
  ...
  if (num_read != 1) {
    my_printf("Error:  %s option requires a float parameter.\n\n", argv[iarg]);
    throw std::runtime_error("1");
  }
...
void Main::reset_buffer() {
    ...
    my_printf("Reading the placement from file %s.\n", place_file_);
}
