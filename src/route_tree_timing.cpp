...
/* Allocates any structures needed to build the routing trees. */
 ...
 if (rr_node_to_rt_node != NULL || rt_node_free_list != NULL ||
    ...
    my_printf ("Error in alloc_route_tree_timing_structs: old structures "
            "already exist.\n");
    exit (1);
 }
 ...
 * and (via a pointer) the rt_node of the new SINK.                         */
 ...
 if (rr_node[inode].type != SINK) {
    my_printf ("Error in add_path_to_route_tree.  Expected type = SINK (%d).\n", 
             SINK);
    my_printf ("Got type = %d.", rr_node[inode].type);
    exit (1);
 }
#endif
 ...
 * recursion, etc.                                                          */
 ...
 while (linked_rt_edge != NULL) {   /* While SINK not reached. */
    ...
    if (linked_rt_edge->next != NULL) {
       my_printf ("Error in load_new_path_R_upstream: new routing addition is\n"
               "a tree (not a path).\n");
       exit (1);
    }
#endif
