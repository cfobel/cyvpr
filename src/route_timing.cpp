...
/* Returns the largest number of pins on any non-global net.    */
 ...
 for (inet=0;inet<num_nets;inet++) {
    if (is_global[inet] == FALSE) {
       max_pins_per_net = my_max(max_pins_per_net, net[inet].num_pins);
    }
 }
...
/* my_printf ("\nRouting inet: %d\n", inet);  */
 ...
 for (ipin=1;ipin<net[inet].num_pins;ipin++) {  /* For all sinks */
    pin_crit = my_max(max_criticality - net_slack[ipin] / T_crit, 0.);
    pin_crit = pow (pin_crit, criticality_exp);
    pin_crit = my_min(pin_crit, max_criticality);
